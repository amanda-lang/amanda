...
class Analyzer(ast.Visitor):
    ...
    def __init__(self, module):
        ...
        self.scope_depth = 0
        self.c_scope = self.global_scope
        self.c_node = None
        self.c_class = None
    ...
    def error(self, code, **kwargs):
        ...
        raise AmandaError.common_error(message, self.c_node.token.line)
    ...
    def get_type(self, type_node):
        if not type_node:
            return self.c_scope.resolve("vazio")
        type_name = type_node.type_name.lexeme
        type_symbol = self.c_scope.resolve(type_name)
    ...
    def visit_inclua(self, node):
        ...
        existing_mod = self.includes.get(mod_path)
    ...
    def visit_functiondecl(self, node):
        ...
        prev_function = self.c_function
        self.c_function = symbol
        self.visit(node.block, scope)
        self.c_function = prev_function
    ...
    def define_func_scope(self, name, params):
        ...
        scope = symbols.Scope(self.c_scope)
    ...
    def visit_classdecl(self, node):
        ...
        self.define_symbol(klass, self.scope_depth, self.c_scope)
        self.c_scope = symbols.Scope(self.c_scope)
        prev_class = self.c_class
        self.c_class = klass
        klass.members = self.c_scope.symbols
        ...
        node.body.symbols = self.c_scope
        self.c_scope = self.c_scope.enclosing_scope
        self.c_class = prev_class

    def visit_eu(self, node):
        if not self.c_class or not self.c_function:
            self.error(
                "a palavra reservada 'eu' só pode ser usada dentro de um método"
            )
        node.eval_type = self.c_class
        return symbols.VariableSymbol("eu", self.c_class)
    ...
    def visit_para(self, node):
        ...
        sym = symbols.VariableSymbol(name, self.c_scope.resolve("int"))
        scope = symbols.Scope(self.c_scope)
    ...
    def builtin_call(self, name, node):
        if name == "lista":
            ...
            node.eval_type = self.c_scope.resolve("vazio")
